		...
		public boolean effect(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			...
			for (Char ch : affected){
				...
				if (ch.alignment != Char.Alignment.ALLY || !positiveOnly){
					...
					ch.damage(Dungeon.NormalLongRange(5 + Dungeon.scalingDepth() / 4, 10 + Dungeon.scalingDepth() / 4), new Electricity());
					if (ch.isAlive()) {
			...
			//appears to crash the game (actually just closes it)
				...
				try {
					...
					if(Messages.lang() != Languages.ENGLISH){
						//Don't bother doing this joke to none-english speakers, I doubt it would translate.//we still consider the effect valid here though as it's cosmetic anyway
					return false;
				} else {
